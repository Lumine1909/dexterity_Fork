...
public class AxisPair {
    ...
    public AxisPair(Vector x, Vector z) {
        if (!DexUtils.isOrthonormal(x, z)) {
            throw new IllegalArgumentException("Axes must be orthonormal!");
        }
    ...
    public Vector getPitchYawRoll() {
        ...
        if (Double.isNaN(roll_rad) || !Double.isFinite(roll_rad)) {
            roll_rad = 0;
        }
        ...
        if (Double.isNaN(yaw_rad)) {
            yaw_rad = 0;
        }
        if (Double.isNaN(pitch_rad)) {
            pitch_rad = PI2;
        }
