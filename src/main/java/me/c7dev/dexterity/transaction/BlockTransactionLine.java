...
public class BlockTransactionLine {
    ...
    public void refresh(Dexterity plugin) {
        if (to == null || (db != null && !db.getEntity().isValid())) {
            db = plugin.getMappedDisplay(db.getUniqueId());
        }
    }

    public UUID undo() {
        if (!committed || isUndone) {
            return null;
        }
        isUndone = true;
        UUID ret = null;
    ...
    public void redo() {
        ...
        if (to != null) {
            if (db.getEntity() == null || !db.getEntity().isValid()) {
                db = new DexBlock(to);
            } else {
                db.loadState(to);
            }
        } else {
            db.remove(); //TODO test that dex display is set correctly on dexblock after undo/redo transactions
        }
    }
}
