...
public class ClickedBlockDisplay {
    ...
    private final BlockDisplay b;
    private final BlockFace bf_;
    private final Vector offset_;
    private final Vector normal_;
    private final Vector up_dir_;
    private final Vector east_dir_;
    private final Vector south_dir_;
    private final Location loc_;
    ...
    private Vector getNormal(BlockFace f, Vector up, Vector east, Vector south) {
        switch (f) {
            ...
            case DOWN:
                return up.clone().multiply(-1).normalize();
            case SOUTH:
                return south.clone().normalize();
            case NORTH:
                return south.clone().multiply(-1).normalize();
            case EAST:
                return east.clone().normalize();
            case WEST:
                return east.clone().multiply(-1).normalize();
            default:
