...
public class CommandHandler {
    ...
    private final String loclabelPrefix;
    ...
    public int constructList(String[] strs, DexterityDisplay disp, String selected, int i, int level) {
        ...
        line.append(disp.getLabel());
        if (disp.getBlocksCount() > 0) {
            line = new StringBuilder(cc2 + ((disp.getLabel().equals(selected)) ? "§d" : "") + line + "§7: " + cc + DexUtils.locationString(disp.getCenter(), 0) + " (" + disp.getCenter().getWorld().getName() + ")");
        } else {
            line = new StringBuilder(cc2 + ((disp.getLabel().equals(selected)) ? "§d" : "") + "§l" + line + cc + ":");
        }
    ...
    public String getConfigString(String dir, DexSession session, String override_label) {
        ...
        if (override_label != null) {
            ...
            if (session != null && session.getSelected() != null) {
                s = s.replaceAll("\\Q%loclabel%", loclabelPrefix + " " + cc2 + DexUtils.locationString(session.getSelected().getCenter(), 0) + cc);
            } else {
                s = s.replaceAll("\\Q%loclabel%\\E", "");
            }
        }
        return s;
    }
    ...
    public void help(CommandContext ctx, String[] commands_str) {
        ...
        if (attrs.containsKey("page")) {
            ...
            page = Math.max(DexUtils.parseInt(ctx.getArgs()[1]) - 1, 0);
        }
    ...
    public void wand(CommandContext ct) {
        if (!withPermission(ct.getPlayer(), "wand")) {
            return;
        }
    ...
    public void debugCenters(CommandContext ct) {
        if (!ct.getPlayer().hasPermission("dexterity.admin")) {
            return;
        }
        ...
        for (DexBlock db : d.getBlocks()) {
            ...
            if (entity_centers) {
                api.markerPoint(db.getEntity().getLocation(), Color.ORANGE, 6);
            }
        }
    }
    ...
    public void debugTestNear(CommandContext ct) {
        ...
        if (b == null) {
            ct.getPlayer().sendMessage(cc + "None in range");
        } else {
    ...
    public void debugItem(CommandContext ct) {
        ...
        if (schem_name == null) {
            p.sendMessage("§cThere is no Dexterity schematic associated with this item!");
        } else {
            p.sendMessage(cc + "This item is tied to the " + cc2 + schem_name + cc + " item schematic.");
        }
    }
            ...
            switch (session.getEditType()) {
                case CLONE_MERGE -> {
                    if (session.getSecondary() != null) {
                        session.getSecondary().hardMerge(session.getSelected(), ct);
                    }
                }
                case CLONE -> ct.getPlayer().sendMessage(getConfigString("clone-success", session));
            }
            session.finishEdit();
        }
    }
    ...
    public void recenter(CommandContext ct) { //TODO: add -auto to recalculate
        ...
        if (ct.getFlags().contains("auto") || ct.getFlags().contains("reset")) {
            ...
            if (!ct.getFlags().contains("continuous")) {
                DexUtils.blockLoc(loc).add(0.5, 0.5, 0.5);
            }
    ...
    public void align(CommandContext ct) {
        ...
        if (session.getFollowingOffset() != null) {
            ...
            if (!ct.getPlayer().isSneaking()) {
                DexUtils.blockLoc(loc2);
            }
    ...
    public void axis(CommandContext ct) {
        ...
        if (args.length == 1 || args[1].equalsIgnoreCase("show")) {
            ...
            if (args.length <= 2) {
                session.setShowingAxes(session.getShowingAxisType() == null ? AxisType.SCALE : null);
            } else {
                if (args[2].equalsIgnoreCase("rotation")) {
                    session.setShowingAxes(AxisType.ROTATE);
                } else if (args[2].equalsIgnoreCase("scale")) {
                    session.setShowingAxes(AxisType.SCALE);
                } else {
            ...
            p.sendMessage(plugin.getConfigString("unknown-input").replaceAll("\\Q%input%\\E", args[1].toLowerCase()));
        }
    }

    public void reload(CommandContext ct) {
        if (!withPermission(ct.getPlayer(), "admin")) {
            return;
        }

        if (ct.getFlags().contains("saved_displays")) {
            plugin.reloadDisplays();
        } else {
            plugin.reload();
        }
    ...
    public void replace(CommandContext ct) {
        ...
        if (args.length >= 3) {
            ...
            p.sendMessage(getUsage("replace"));
        }
    }
    ...
    public void clone(CommandContext ct) {
        ...
        if (!withPermission(p, "clone") || testInEdit(session)) {
            return;
        }
        DexterityDisplay d = session.getSelected();
        if (d == null && def == null) {
        ...
        if (!clone.getCenter().getWorld().getName().equals(p.getWorld().getName()) || clone.getCenter().distance(p.getLocation()) >= 80) {
            clone.teleport(p.getLocation());
        }
    ...
    public void owner(CommandContext ct) {
        ...
        if (args.length == 1 || args[1].equalsIgnoreCase("list")) {
            ...
            if (owners.length == 0) {
                owners_str.add(cc + "- " + cc2 + "*");
            } else {
                for (OfflinePlayer owner : owners) {
                    owners_str.add(cc + "- " + cc2 + owner.getName());
                }
            }
            ...
            if (d.getOwners().length == 0) {
                p.sendMessage(getConfigString("owner-remove-success-warning", session));
            }
        } else {
    ...
    public void tile(CommandContext ct) {
        ...
        if (attrs_d.containsKey("x")) {
            delta.setX(attrs_d.get("x"));
        }
        if (attrs_d.containsKey("y")) {
            delta.setY(attrs_d.get("y"));
        }
        if (attrs_d.containsKey("z")) {
            delta.setZ(attrs_d.get("z"));
        }
        if (attrs_d.containsKey("rx") || attrs_d.containsKey("ry") || attrs_d.containsKey("rz") || def != null) {
            ...
            if (attrs_d.containsKey("rx")) {
                delta.add(rot.getXAxis().multiply(attrs_d.get("rx")));
            }
            if (attrs_d.containsKey("ry")) {
                delta.add(rot.getYAxis().multiply(attrs_d.get("ry")));
            }
            if (attrs_d.containsKey("rz")) {
                delta.add(rot.getZAxis().multiply(attrs_d.get("rz")));
            }
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            toMerge.hardMerge(c, ct);
        }

        d.hardMerge(toMerge, ct);
        t.commit();
        session.pushTransaction(t);
    ...
    public void glow(CommandContext ct) { //TODO: add transaction
        ...
        if (d.getLabel() != null) {
            p.sendMessage(getConfigString("glow-success", session));
        }
    }
    ...
    public void seat(CommandContext ct) {
        ...
        if (anim == null) {
            ...
            if (y_offset != 0) {
                a.setSeatOffset(new Vector(0, y_offset, 0));
            }
            d.addAnimation(a);
    ...
    public void command(CommandContext ct) {
        ...
        if (args[1].equalsIgnoreCase("add")) {
            ...
            for (int i = 2; i < args.length; i++) {
                String arg = args[i];
                if (!appending && !arg.contains("=") && !arg.startsWith("-") && !arg.contains(":")) {
                    appending = true;
                }
                if (appending) {
                    cmd_strb.append(arg);
            ...
            if (attr_str.containsKey("permission")) {
                command.setPermission(attr_str.get("permission"));
            }
            ...
            if (index < 0) {
                index = d.getCommandCount() + index + 1; //ex. input -1 for the last command
            }
            ...
            if (d.getCommandCount() == 0) {
                p.sendMessage(getConfigString("list-empty", session));
            } else {
    ...
    public void convert(CommandContext ct) {
        DexSession session = ct.getSession();
        Player p = ct.getPlayer();
        if (!withPermission(p, "convert") || testInEdit(session)) {
            return;
        }
        ...
        if (l1 != null && l2 != null) {
            ...
            DexterityDisplay d = api.convertBlocks(l1, l2, t, (int) vol + 1, ct);

            session.setCancelPhysics(false);
            if (session.getSelected() != null && !session.getSelected().isSaved() && session.getSelected().getCenter().getWorld().getName().equals(d.getCenter().getWorld().getName())) {
                session.getSelected().hardMerge(d, ct); //within cuboid selection
            } else {
                session.setSelected(d, false);
            }
            ...
            p.sendMessage(getConfigString("need-locations", session));
        }
    }
    ...
    public void move(CommandContext ct) {
        ...
        BlockTransaction t = new BlockTransaction(d);
        Location loc;
        if (flags.contains("continuous") || flags.contains("c")) {
            loc = p.getLocation();
        } else if (flags.contains("here")) {
            loc = DexUtils.blockLoc(p.getLocation()).add(0.5, 0.5, 0.5);
        } else {
            loc = d.getCenter();
        }

        HashMap<String, Double> attr_d = ct.getDoubleAttrs();
        if (attr_d.containsKey("x")) {
            loc.add(attr_d.get("x"), 0, 0);
        }
        if (attr_d.containsKey("y")) {
            loc.add(0, attr_d.get("y"), 0);
        }
        if (attr_d.containsKey("z")) {
            loc.add(0, 0, attr_d.get("z"));
        }
        if (attr_d.containsKey("rx") || attr_d.containsKey("ry") || attr_d.containsKey("rz")) {
            ...
            if (attr_d.containsKey("rx")) {
                loc.add(x.multiply(attr_d.get("rx")));
            }
            if (attr_d.containsKey("ry")) {
                loc.add(y.multiply(attr_d.get("ry")));
            }
            if (attr_d.containsKey("rz")) {
                loc.add(z.multiply(attr_d.get("rz")));
            }
        }
    ...
    public void save(CommandContext ct) {
        ...
        if (args.length != 2) {
            if (args[0].equals("save") || args[0].equals("label")) {
                p.sendMessage(getUsage("rename").replaceAll(" name", " label"));
            } else {
                p.sendMessage(getUsage("rename"));
            }
            return;
        }
        ...
        if (d.setLabel(args[1])) {
            ...
            p.sendMessage(getConfigString("name-in-use", session).replaceAll("\\Q%input%\\E", args[1]));
        }
    }
    ...
    public void undo(CommandContext ct) {
        Location loc = ct.getPlayer().getLocation();
        if (!IntegrationManager.canModify(
            ct.getPlayer(), ct.getPlayer().getWorld().getName(),
            loc.x(), loc.y(), loc.z()
        )) {
            ...
            switch (Math.min(ct.getDefaultArgs().size(), 6)) {
                case 6:
                    if (plan.zDeg == Double.MAX_VALUE) {
                        plan.zDeg = Double.parseDouble(defs_n.get(5));
                    }
                case 5:
                    if (plan.xDeg == Double.MAX_VALUE) {
                        plan.xDeg = Double.parseDouble(defs_n.get(4));
                    }
                case 4:
                    if (plan.yawDeg == Double.MAX_VALUE) {
                        plan.yawDeg = Double.parseDouble(defs_n.get(3));
                    }
                case 3:
                    if (plan.rollDeg == Double.MAX_VALUE) {
                        plan.rollDeg = Double.parseDouble(defs_n.get(2));
                    }
                case 2:
                    if (plan.pitchDeg == Double.MAX_VALUE) {
                        plan.pitchDeg = Double.parseDouble(defs_n.get(1));
                    }
                case 1:
                    if (plan.yDeg == Double.MAX_VALUE) {
                        plan.yDeg = Double.parseDouble(defs_n.get(0));
                    }
                default:
            }
        } catch (Exception ex) {
    ...
    public void schematic(CommandContext ct) {
        ...
        if (args.length == 1) {
            ...
            if (!withPermission(p, "schem.import") || testInEdit(session)) {
                return;
            }
            ...
            if (!withPermission(p, "schem.export")) {
                return;
            }
            DexterityDisplay d = getSelected(session, "export");
            if (d == null || testInEdit(session)) {
                return;
            }
            ...
            HashMap<String, String> attr_str = ct.getStringAttrs();
            if (attr_str.containsKey("author")) {
                author = attr_str.get("author");
            }
            ...
            if (attrs.containsKey("page")) {
                ...
                page = Math.max(DexUtils.parseInt(args[2]) - 1, 0);
            }
            ...
            for (int i = 0; i < schems.size(); i++) {
                formatted[i] = cc + "- " + cc2 + schems.get(i);
            }
    ...
    public void info(CommandContext ct) {
        ...
        if (ct.getFlags().contains("scale")) {
            ...
            if (d.getLabel() != null) {
                msg = msg.replaceAll("\\Q%label%\\E", d.getLabel());
            }
    ...
    public void remove(CommandContext ct) {
        ...
        if ((res && !withPermission(p, "remove")) || (!res && !withPermission(p, "deconvert"))) {
            return;
        }

        if (testInEdit(session)) {
            return;
        }
        ...
        if (res) {
            ...
            p.sendMessage(getConfigString("remove-success", session, label));
        }
    ...
    public void merge(CommandContext ct) {
        ...
        if (hard) {
            ...
            if (g != null) {
                ...
                if (new_group == null) {
                    p.sendMessage(getConfigString("merge-success", session).replaceAll("\\Q%parentlabel%\\E", parent.getLabel()));
                } else {
                    p.sendMessage(getConfigString("merge-success-newgroup", session).replaceAll("\\Q%input%\\E", new_group));
                }
            } else {
                p.sendMessage(getConfigString("failed-merge", session));
            }
