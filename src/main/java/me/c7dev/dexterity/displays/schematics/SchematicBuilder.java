...
public class SchematicBuilder {
    ...
    private final HashMap<DoubleKey, DoubleToken> doubleMap = new HashMap<>();
    private final HashMap<StringKey, StringToken> stringMap = new HashMap<>();
    private final HashMap<Character, CharToken> charMap = new HashMap<>();
    private final HashMap<TokenType, Token> specifierMap = new HashMap<>();

    private final List<Token> encodedOutput = new ArrayList<>();
    private final HashMap<Token, Integer> freq = new HashMap<>();
    ...
    public int save(String fileName, String author, boolean override) {
        ...
        if (author.contains(";")) {
            throw new IllegalArgumentException("Author name cannot have ; in it!");
        }
        try {
    ...
    public boolean save(File f, String author) {
        ...
        if (!assignedTags) {
            throw new DexterityException("Did not assign tags to tokens, cannot save!");
        }
        ...
        try {
            ...
            for (int i = 32; i <= 254; i++) {
                ...
                if (token != null) {
                    ...
                    if (token.getTag() == null) {
                        writer.close();
                        throw new DexterityException("Tag is undefined: " + token);
                    }
            ...
            for (Token token : encodedOutput) {
                encoder.append(token.getTag());
            }
    ...
    private void encodeBlocks(DexterityDisplay d) {
        ...
        for (DexBlock db : d.getBlocks()) {
            ...
            if (Math.abs(diff.getX()) >= epsilon) {
                addToken(getDouble(TokenType.DX, diff.getX()));
            }
            if (Math.abs(diff.getY()) >= epsilon) {
                addToken(getDouble(TokenType.DY, diff.getY()));
            }
            if (Math.abs(diff.getZ()) >= epsilon) {
                addToken(getDouble(TokenType.DZ, diff.getZ()));
            }

            double roll = db.getRoll() % 360;
            if (Math.abs(db.getEntity().getLocation().getYaw()) >= epsilon) {
                addToken(getDouble(TokenType.YAW, db.getEntity().getLocation().getYaw()));
            }
            if (Math.abs(db.getEntity().getLocation().getPitch()) >= epsilon) {
                addToken(getDouble(TokenType.PITCH, db.getEntity().getLocation().getPitch()));
            }
            if (Math.abs(roll) >= epsilon) {
                addToken(getDouble(TokenType.ROLL, roll));
            }

            if (Math.abs(db.getTransformation().getScale().getX() - 1) >= epsilon) {
                addToken(getDouble(TokenType.SCALE_X, db.getTransformation().getScale().getX()));
            }
            if (Math.abs(db.getTransformation().getScale().getY() - 1) >= epsilon) {
                addToken(getDouble(TokenType.SCALE_Y, db.getTransformation().getScale().getY()));
            }
            if (Math.abs(db.getTransformation().getScale().getZ() - 1) >= epsilon) {
                addToken(getDouble(TokenType.SCALE_Z, db.getTransformation().getScale().getZ()));
            }

            if (Math.abs(db.getTransformation().getDisplacement().getX() + (0.5 * db.getTransformation().getScale().getX())) >= epsilon) {
                addToken(getDouble(TokenType.TRANS_X, db.getTransformation().getDisplacement().getX()));
            }
            if (Math.abs(db.getTransformation().getDisplacement().getY() + (0.5 * db.getTransformation().getScale().getY())) >= epsilon) {
                addToken(getDouble(TokenType.TRANS_Y, db.getTransformation().getDisplacement().getY()));
            }
            if (Math.abs(db.getTransformation().getDisplacement().getZ() + (0.5 * db.getTransformation().getScale().getZ())) >= epsilon) {
                addToken(getDouble(TokenType.TRANS_Z, db.getTransformation().getDisplacement().getZ()));
            }

            if (Math.abs(db.getTransformation().getRollOffset().getX()) >= epsilon) {
                addToken(getDouble(TokenType.ROFFSET_X, db.getTransformation().getRollOffset().getX()));
            }
            if (Math.abs(db.getTransformation().getRollOffset().getY()) >= epsilon) {
                addToken(getDouble(TokenType.ROFFSET_Y, db.getTransformation().getRollOffset().getY()));
            }
            if (Math.abs(db.getTransformation().getRollOffset().getZ()) >= epsilon) {
                addToken(getDouble(TokenType.ROFFSET_Z, db.getTransformation().getRollOffset().getZ()));
            }

            if (Math.abs(db.getTransformation().getLeftRotation().x) >= epsilon) {
                addToken(getDouble(TokenType.QUAT_X, db.getTransformation().getLeftRotation().x));
            }
            if (Math.abs(db.getTransformation().getLeftRotation().y) >= epsilon) {
                addToken(getDouble(TokenType.QUAT_Y, db.getTransformation().getLeftRotation().y));
            }
            if (Math.abs(db.getTransformation().getLeftRotation().z) >= epsilon) {
                addToken(getDouble(TokenType.QUAT_Z, db.getTransformation().getLeftRotation().z));
            }
            if (Math.abs(db.getTransformation().getLeftRotation().w - 1) >= epsilon) {
                addToken(getDouble(TokenType.QUAT_W, db.getTransformation().getLeftRotation().w));
            }
    ...
    private List<Token> createObjectTokens(boolean create_list) {
        ...
        for (Entry<DoubleKey, DoubleToken> entry : doubleMap.entrySet()) {
            ...
            if (create_list) {
                addString("" + token.getTag().length, defs);
            }
