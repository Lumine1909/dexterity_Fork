...
public class DexterityCommand implements CommandExecutor, TabCompleter {
    ...
    private final String[] command_strs = new String[commands.length];
    ...
    public boolean cmdDelay(Player p) {
        ...
        new BukkitRunnable() {
            @Override
            public void run() {
                if (cmdDelay.getOrDefault(u, 0L) == newdelay) {
                    cmdDelay.remove(u);
                }
            }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!p.hasPermission("dexterity.nocmddelay") && cmdDelay(p)) {
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] argsr) {
        ...
        for (String s : argsr) {
            ...
            if (ssplit.length > 0) {
                params.add(DexUtils.attrAlias(ssplit[0]));
            }
        }

        List<String> ret = new ArrayList<String>();
        if (!sender.hasPermission("dexterity.command")) {
            return ret;
        }

        if (argsr.length <= 1) {
            Collections.addAll(ret, commands);
            ret.add("cancel");
            ret.add("paste");
        }
        ...
        switch (argsr[0]) {
            ...
            case "schematic":
                ...
                if (argsr.length == 2) {
                    ...
                    } else if (argsr[1].equalsIgnoreCase("list") || argsr[1].equalsIgnoreCase("lsit")) {
                        ret.add("page=");
                    }
                }
                return ret;
        ...
        switch (argsr[0]) {
            ...
            case "rm":
                Player p = (Player) sender;
                for (String s : plugin.getDisplayLabels(p)) {
                    ret.add(s);
                }
                return ret;
        }
    ...
    private void addCmdIDs(List<String> ret, CommandSender sender) {
        ...
        if (session != null && session.getSelected() != null) {
            len = session.getSelected().getCommandCount();
        }
        for (int i = 1; i <= len; i++) {
            ret.add("" + i);
        }
