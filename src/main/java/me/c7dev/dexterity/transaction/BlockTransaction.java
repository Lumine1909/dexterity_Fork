...
public class BlockTransaction implements Transaction {
    ...
    public BlockTransaction(DexterityDisplay disp, Mask mask) {
        if (disp == null) {
            throw new IllegalArgumentException("Display cannot be null on transaction!");
        }
        this.disp = disp;
        oldCenter = disp.getCenter();
        if (mask == null) {
            for (DexBlock db : disp.getBlocks()) {
                trans.put(db.getEntity().getUniqueId(), new BlockTransactionLine(db));
            }
        } else {
            for (DexBlock db : disp.getBlocks()) {
    ...
    public void addBlock(DexBlock block) {
        if (trans.containsKey(block.getEntity().getUniqueId())) {
            return;
        }
    ...
    public void includeRemovedBlocks() {
        for (Entry<UUID, BlockTransactionLine> entry : trans.entrySet()) {
            if (!entry.getValue().isCommitted() && Bukkit.getEntity(entry.getKey()) == null) {
                entry.getValue().commit(null);
            }
        }
    }
    ...
    public DexterityDisplay undo() {
        ...
        if (newCenter != null && oldCenter != null) {
            disp.setCenter(oldCenter);
        }
        return null;
    }
