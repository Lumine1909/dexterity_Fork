...
public class Schematic {
    ...
    public Schematic(Dexterity plugin, String file_name) {
        ...
        if (!file_name.endsWith("\\.dexterity")) {
            file_name += ".dexterity";
        }
        this.plugin = plugin;
        try {
        ...
        try {
            ...
            if (f.exists()) {
                ...
                for (String req : req_sections) {
                    if (!schem.contains(req)) {
                        throw new DexterityException("Schematic must include '" + req + "' section!");
                    }
                }
                ...
                if (hashval == null) {
                    throw new DexterityException("Schematic is missing hash");
                }
                ...
                if (!hashread.equals(hashval)) {
                    throw new DexterityException("Could not load schematic: Hashes do not match");
                }
    ...
    public List<Token> decode(byte[] data) {
        ...
        while (true) {
            ...
            if (curr.getLeafToken() != null) {
                ...
                if (curr.getLeafToken().getType() == TokenType.DATA_END) {
                    break; //eof
                }
                curr = root;
                continue;
    ...
    public void decodeObjects(byte[] data) {
        ...
        while (true) { //Encoder format (v1, base 10): [<tag len>] <type> [<val chars>] [tag] <block delimiter>
            if (state == DecodeState.NOT_STARTED) {
                state = DecodeState.SEEK_TAG_LENGTH; //first bit - may need to run code later
            } else {

                if (state == DecodeState.GET_TAG) { //read binary tag
                    if (toLeft) {
                        tagread.bits.set(tagread_index);
                    }
                    tagread_index++;
                    if (tagread_index == taglen) {
                        state = DecodeState.SEEK_VALUE; //read value
                    }
                } else {
                    if (curr == null) {
                    ...
                    if (curr.getLeafToken() != null) {
                        ...
                        if (token.getType() == TokenType.ASCII) {
                            ...
                            if (state == DecodeState.SEEK_TAG_LENGTH || state == DecodeState.SEEK_TYPE) { //tag length int
                                if (ctk.getCharValue() < 48 || ctk.getCharValue() > 57) {
                                    throw new DexterityException("Malformed objects header: Expected tag length to be number");
                                }
                        ...
                        } else if (token.getType() == TokenType.DATA_END) {
                            return;
                            ...
                            throw new DexterityException("Malformed objects header: Token sequencing is incorrect for object definition (" + state + ")");
                        }
    ...
    private void load(YamlConfiguration schem) {
        if (loaded) {
            throw new DexterityException("This schematic is already loaded!");
        }
        ...
        for (String charstr : charset) {
            ...
            Token token;
            if (index <= 255) {
                token = new CharToken((char) index);
            } else {
                TokenType type = typevals[index - 256];
                token = new Token(type);
                if (type == TokenType.DATA_END) {
        ...
        if (!found_end_token) {
            throw new DexterityException("Data does not contain required tokens!");
        }
    ...
    public void reloadBlocks(List<Token> data) { //block data interpreter
        ...
        for (Token t : data) {
            ...
            if (t instanceof DoubleToken dt) {
    ...
    public DexterityDisplay paste(Location loc) {
        if (!loaded) {
            throw new DexterityException("This schematic did not load! Cannot paste.");
        }
        loc = loc.clone();
