...
public class DexterityDisplay {
    ...
    public boolean equals(Object o) {
        if (!(o instanceof DexterityDisplay d)) {
            return false;
        }
    ...
    public OfflinePlayer[] getOwners() {
        ...
        for (int i = 0; i < owners.size(); i++) {
            r[i] = Bukkit.getOfflinePlayer(owners.get(i));
        }
        return r;
    }
    ...
    public void setOwners(List<OfflinePlayer> newOwners) {
        ...
        for (OfflinePlayer o : newOwners) {
            owners.add(o.getUniqueId());
        }
    }
    ...
    public boolean hasOwner(Player p) {
        if (owners == null || owners.size() == 0 || p.hasPermission("dexterity.select.unowned")) {
            return true;
        }
    ...
    public void addOwner(OfflinePlayer p) {
        if (p == null || owners.contains(p.getUniqueId())) {
            return;
        }
        ...
        if (blocks.size() == 0) {
            ...
            for (DexBlock db : blocks) {
                ...
                if (rescale) {
                    ...
                    if (scale.getX() > scalex) {
                        scalex = scale.getX();
                    }
                    if (scale.getY() > scaley) {
                        scaley = scale.getY();
                    }
                    if (scale.getZ() > scalez) {
                        scalez = scale.getZ();
                    }
                }

                if (zeroPitch && db.getEntity().getLocation().getPitch() != 0) {
                    zeroPitch = false;
                }
            }
            if (rescale) {
    ...
    public void setDefaultLabel() {
        int i = 1;
        while (plugin.getDisplayLabels().contains("display-" + i)) {
            i++;
        }
        setLabel("display-" + i);
    }
    ...
    public boolean setLabel(String s) {
        ...
        if (s.startsWith("-") || s.contains(".") || s.contains(" ")) {
            throw new IllegalArgumentException("Invalid label! Cannot start with '-' or contain '.' or ' ' symbols.");
        }

        if (plugin.getDisplayLabels().contains(s)) {
            return false;
        }
        if (label != null) {
    ...
    public void unregister() {
        ...
        label = null;
        for (DexterityDisplay sub : subdisplays) {
            sub.unregister();
        }
    }
    ...
    public void addBlock(DexBlock db) {
        if (db.getDexterityDisplay() != null && db.getDexterityDisplay().isSaved() && !uuid.equals(db.getDexterityDisplay().getUniqueId())) {
            db.getDexterityDisplay().removeBlockNoUnmap(db);
        }
        db.setDexterityDisplay(this);
        blocks.add(db);
    }
    ...
    public void addAnimation(Animation a) {
        ...
        if (existing == null && a instanceof RideableAnimation) {
            existing = getAnimation(RideableAnimation.class);
        }
        if (existing != null) {
    ...
    public void setDropItem(ItemStack item, String schemName) {
        if (!isSaved()) {
            throw new RuntimeException("Can not set display's drop item when display is not saved.");
        }
        if (item == null) {
    ...
    private void updateDropItemMeta() {
        if (item == null || itemSchemLabel == null || label == null) {
            return;
        }
        ...
        meta.setDisplayName("§f" + disp_label.toUpperCase().charAt(0) + disp_label.substring(1));
    ...
    public void addSubdisplay(DexterityDisplay d) {
        if (!this.isSaved() || !d.isSaved()) {
            throw new IllegalArgumentException("Both displays must be saved to add a subdisplay!");
        }
        if (d.getParent() != null) {
            d.getParent().removeSubdisplay(d);
        }
        subdisplays.add(d);
    }
     ...
     */
    public boolean hardMerge(DexterityDisplay subdisplay, CommandContext ct) {
        if (!subdisplay.getCenter().getWorld().getName().equals(center.getWorld().getName()) ||
            !subdisplay.canHardMerge() || !canHardMerge()) {
            return false;
        }
        ...
        for (DexBlock b : subdisplay.getBlocks()) {
            ...
            blocks.add(b);
            if (zeroPitch && b.getEntity().getLocation().getPitch() != 0) {
                zeroPitch = false;
            }
        }
    ...
    public DexterityDisplay merge(DexterityDisplay newParent, String newGroup) {
        if (newParent == this || newParent.getLabel().equals(label) || subdisplays.contains(newParent) || parent != null) {
            throw new IllegalArgumentException("Cannot merge with self!");
        }
        if (rootDisplay(this).containsSubdisplay(newParent)) {
            throw new IllegalArgumentException("One display must be a root node to be able to merge!");
        }
        if (!newParent.getCenter().getWorld().getName().equals(center.getWorld().getName())) {
            throw new IllegalArgumentException("Both displays must be in the same world!");
        }
        if (newGroup != null && plugin.getDisplayLabels().contains(newGroup)) {
            throw new IllegalArgumentException("New group label is already in use!");
        }
        if (!isSaved() || !newParent.isSaved()) {
            throw new IllegalArgumentException("Both displays must be saved!");
        }
    ...
    private void removeHelper(boolean restore) {
        ...
        for (DexterityDisplay subdisplay : subdisplays.toArray(new DexterityDisplay[subdisplays.size()])) {
            subdisplay.remove(restore);
        }
    }
    ...
    private int getGroupSize(DexterityDisplay s) {
        int i = 1;
        for (DexterityDisplay sub : s.getSubdisplays()) {
            i += getGroupSize(sub);
        }
        return i;
    }
    ...
    public void setCenter(Location loc) {
        if (center != null && !loc.getWorld().getName().equals(center.getWorld().getName())) {
    ...
    public void teleport(Location loc) {
        ...
        if (loc.getWorld().getName().equals(center.getWorld().getName())) {
            ...
            center = loc.clone();
            for (DexterityDisplay subd : subdisplays) {
                subd.teleport(loc);
            }
        }
    ...
    public void scale(double s) {
        ...
        for (DexBlock db : blocks) {
            ...
            if (db.getTransformation().getRollOffset() != null) {
                db.getTransformation().getRollOffset().multiply(s);
            }
    ...
    public void scale(Vector v) {
        ...
        for (DexBlock db : blocks) {
            ...
            if (all_key == null) {
                all_key = key;
            } else if (!all_key.equals(key)) {
                throw new DexterityException("This selection has too many rotation orientation types to skew!");
            }
        }
    ...
    public void align(boolean toCenter, boolean x, boolean y, boolean z) {
        if (!(x || y || z)) {
            throw new IllegalArgumentException("Must align in at least one axis");
        }
        Vector diff;
        if (toCenter) {
    ...
    public void setBaseRotation(float yaw, float pitch, float roll) {
        if (rot == null) {
            rot = new DexRotation(this, yaw, pitch, roll);
        } else {
            rot.setAxes(yaw, pitch, roll);
        }
    }
        ...
        blocks.sort((l, r) -> {
            ...
            if (Math.abs(l1.getY() - l2.getY()) < epsilon) {
                if (Math.abs(l1.getX() - l2.getX()) < epsilon) {
                    if (Math.abs(l1.getZ() - l2.getZ()) < epsilon) {
                        return 0;
                    }
     ...
     */
    public void consolidate(Mask m, BlockTransaction t, CommandContext ct) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            for (DexBlock db : blocks) {
                if (m != null && !m.isAllowed(db.getEntity().getBlock().getMaterial())) {
                    continue;
                }
            ...
            Bukkit.getScheduler().runTask(plugin, () -> { //sync thread to send packets
                for (DexBlock db : toRemove) {
                    db.remove();
                }
