...
public class DexSession {
    ...
    public DexSession(Player player, Dexterity plugin) {
        if (player == null || plugin == null || !player.isOnline()) {
    ...
    public void setSelected(DexterityDisplay o, boolean msg) {
        ...
        if (selected != null && o != null) {
            editingLock = o.getEditingLock();
        }
        if (editingLock != null) {
            Player editor = Bukkit.getPlayer(editingLock);
            if (editor == null) {
                o.setEditingLock(null);
            } else {
                if (editingLock.equals(p.getUniqueId())) {
                    p.sendMessage(plugin.getConfigString("must-finish-edit"));
                } else {
                    p.sendMessage(plugin.getConfigString("cannot-select-with-edit").replaceAll("\\Q%editor%\\E", editor.getName()));
                }
                return;
            }
        }
    ...
    public void pushBlock(DexBlock db, boolean placing) {
        if (db.getDexterityDisplay() == null) {
            return;
        }
        if (buildTrans == null) {
        ...
        if (placing) {
            buildTrans.addBlock(db);
        } else {
            buildTrans.removeBlock(db);
        }
    }
    ...
    private void executeUndo(int count) {
        ...
        if (toUndo.size() == 0) {
            if (count > 0) {
                p.sendMessage(plugin.getConfigString("none-undo"));
            }
            return;
        }
        ...
        if (!undo.isPossible()) {
            if (count > 0) {
                p.sendMessage(plugin.getConfigString("cannot-undo"));
            }
            return;
        }
    ...
    private void executeRedo(int count) {
        if (toRedo.size() == 0) {
            if (count > 0) {
                p.sendMessage(plugin.getConfigString("none-redo"));
            }
            return;
        }
        ...
        if (!redo.isPossible()) {
            if (count > 0) {
                p.sendMessage(plugin.getConfigString("cannot-redo"));
            }
            return;
        }
    ...
    public void startEdit(DexterityDisplay d, EditType type, boolean swap, Transaction t) {
        if (selected == null || editType != null) {
            return;
        }
        editType = type;
        editTransaction = t;
        if (d != selected) {
            ...
            selected.setEditingLock(p.getUniqueId());
        }
        origLoc = d.getCenter();
    }
    ...
    public void cancelEdit() {
        ...
        if (editType == EditType.TRANSLATE && origLoc != null) {
            if (secondary != null) {
                secondary.teleport(origLoc);
            } else {
                selected.teleport(origLoc);
            }
        }
    ...
    public void setLocation(Location loc, boolean isL1, boolean verbose) {
        ...
        if (hasLocationsSet()) {
            volume = DexUtils.getBlockVolume(l1, l2);
        }
    }
...
//		else scale.multiply(0.5);
        ...
        if (verbose) {
            p.sendMessage(plugin.getConfigString("set-success").replaceAll("\\Q%number%\\E", isL1 ? "1" : "2").replaceAll("\\Q%location%\\E", DexUtils.locationString(loc, 0)));
        }
    }
    ...
    public void updateAxisDisplays() {
        removeAxes();
        if (selected == null || showingAxis == null) {
            return;
        }
    ...
    public void removeAxes() {
        DexterityAPI api = plugin.api();
        if (axisX != null) {
            api.removeMarker(axisX);
        }
        if (axisY != null) {
            api.removeMarker(axisY);
        }
        if (axisZ != null) {
            api.removeMarker(axisZ);
        }
        axisX = null;
        axisY = null;
        axisZ = null;
